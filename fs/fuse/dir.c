...
static int fuse_submount_test_super(struct super_block *sb,
				    struct fs_context *fsc)
{
	...
	return fsc_fm->fc->iq.priv == sb_fm->fc->iq.priv &&
		fsc_fm->submount_dev == sb_fm->submount_dev;
}
...
static struct vfsmount *fuse_dentry_automount(struct path *path)
{
	...
	fm->submount_dev = mp_finode->submount_dev;

	/* Get (new) superblock for this new fuse_mount */
	fsc->s_fs_info = fm;
	sb = sget_fc(fsc, fuse_submount_test_super, fuse_submount_set_super);
	fuse_mount_put(fm);
	if (IS_ERR(sb)) {
		err = PTR_ERR(sb);
		goto out_put_fsc;
